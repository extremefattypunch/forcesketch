...
\section*{SIMBIOCHEM II --- please read before submitting}
This is the submission template for the \textbf{2nd SIMBIOCHEM Workshop at NeurIPS 2026} (Sydney, Australia). The workshop is \textbf{non-archival} and \textbf{double-blind}.
\begin{itemize}
  \item \textbf{Length.} Papers are \textbf{5 to 8 pages}. References and appendices are \textbf{unlimited} and do not count towards the limit; reviewers are under \textbf{no obligation to read beyond 8 pages}.
  \item \textbf{Anonymity.} Submit anonymised: no author names, affiliations, acknowledgements, or identifying links (GitHub, personal sites, or identifying figures/metadata). Non-anonymised papers may be \textbf{desk-rejected}. As the venue is non-archival, reviewers are instructed to evaluate only the PDF and not to search for external sources.
  \item \textbf{No rebuttals.} There is no author-response phase.
  \item \textbf{AI / LLM disclosure.} Disclose any use of AI/LLMs in your research or writing; you remain fully responsible for the content.
  \item \textbf{Research integrity.} Submitting the same paper more than once (tweaked duplicates or multiple versions) is misconduct. \textbf{We run mechanisms to detect this}; violations are reported to OpenReview and NeurIPS and result in desk rejection. Any attempt to game the review process is reported to the organisers, NeurIPS, and OpenReview.
  \item \textbf{Checklist.} Completing the checklist at the end is \textbf{highly encouraged but not enforced} --- omitting it is \emph{not} grounds for desk rejection.
  \item \textbf{Camera-ready.} Accepted papers are invited to submit a camera-ready version; compile with \texttt{[dblblindworkshop, final]}.
\end{itemize}

\medskip
The remaining sections reproduce the standard NeurIPS formatting instructions.
