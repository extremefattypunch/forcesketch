...
% ICBINB-BIO (NeurIPS 2026) build. The workshop ships its OWN copy of
% neurips_2026.sty, whose footer differs from the generic one ("Submitted to the
% <workshop> workshop at NeurIPS 2026." rather than "... Do not distribute."), so
% this file is compiled against paper/template_icbinb/ via TEXINPUTS rather than
% against the copy in paper/. The two lines below are verbatim what the
% workshop's own neurips_2026.tex instructs authors to use. Submissions are
% double-blind; `dblblindworkshop' leaves \if@anonymous true.
...
\begin{abstract}
Machine-learned interatomic potentials simulate molecules at a small fraction of the cost of quantum
chemistry, but they are only useful if they can tell us when they are guessing. The standard way to
ask is to train a \emph{committee} of predictors and measure how much they disagree. A
\emph{multi-head} committee makes this almost free for energies: $M$ prediction heads share one
network body, so a single forward pass through the network returns all $M$ energy estimates at once.
Forces are not free. A force is the derivative of the energy with respect to the atomic positions,
and recovering each head's forces takes its own backward pass through that network --- so force
disagreement, the quantity that actually decides which structures a simulation should stop and
verify, costs roughly $M$ times more than energy disagreement.

We ask whether a handful of cheap random probes can estimate that disagreement without computing all
$M$ sets of forces, using a pretrained eight-head committee on four molecular systems. The answer is
no for \emph{replacement} and yes for \emph{screening}. Even given six of the seven probes the exact
computation would use, the best estimator identifies only
\fsMaxRecallKSixLoPct--\fsMaxRecallKSixHiPct\% of the genuinely most uncertain structures, so an
approximate score cannot stand in for the exact one. But a calibrated filter built from that same
score safely skips \fsGateCVSkipLoPct--\fsGateCVSkipHiPct\% of the expensive computations while
still catching \fsGateCVRecallLoPct--\fsGateCVRecallHiPct\% of the uncertain structures, and beats
cheaper
alternatives in \fsGateCVDominates{} of \fsGateCVComparisons{} comparisons. Whether this saves
wall-clock time turns out to depend on how the model is run: filtering is worth
$\fsBestTotalKThreeBSixteen\times$ when structures are processed in batches, but only
$\fsBestTotalKThreeBOne\times$ one at a time, where it becomes a net slowdown. The decision-quality
result is robust; the speed result is not. We report this as a failure mode rather than a
fix: the estimator that looks correct in expectation is the one that fails at the decision, and it
is calibration, not a better estimator, that recovers a usable guarantee.
\end{abstract}
...
The failure we report is a failure of \emph{unbiasedness as a proxy for usefulness}. Every estimator
below is unbiased for the force-variance statistic, with closed-form finite-$K$ corrections, and each
is still unusable as a drop-in replacement, because the decision downstream is a rank or a threshold
rather than a mean and an unbiased score with the wrong noise structure gets that decision wrong. Two
limits are worth stating plainly. The systems are small drug-like molecules of 9--24 atoms, not
proteins or assemblies, so the extreme-value bias we characterise over $3N$ noisy per-component
estimates should be read as a lower bound on what the same estimator would suffer at biomolecular
scale, where $N$ is orders of magnitude larger. And the one clearly positive result, the calibrated
gate, inherits the coverage guarantee of split conformal prediction, which holds under
exchangeability; a molecular dynamics trajectory is not exchangeable, so the guarantee applies to the
held-out frames we evaluate on and not automatically to frames drawn sequentially from a running
simulation.
...
We formulate multi-head force disagreement as a statistic of the centered energy Jacobian and show
the exact computation needs only the centered $(M{-}1)$-dimensional head subspace; we give unbiased
Gaussian, Haar-orthogonal, Rademacher and pairwise estimators with closed-form finite-$K$
corrections, correcting a removable singularity in the Haar constant. We then report a
\emph{negative} result we believe is the useful one --- at $K\le4$ none of these preserves the exact
top-5\% acquisition set, on any of four systems --- and show what does work: an empirically
calibrated screening gate over a low-rank control variate, whose decision quality and wall-clock
saving we quantify separately, because only the first survives.
...
Ranges are across the four systems; per-system values with $95\%$ intervals are in the artifact.
The control variate \emph{Pareto-dominates} --- strictly more skipped \emph{and} strictly higher
recall, not a trade --- in \fsGateCVDominates{} of the \fsGateCVComparisons{} gate-by-system
comparisons, including every one against the two sketch baselines. The exception is worth stating
plainly: on ethanol the free energy gate attains perfect recall (\fsGateEnergyRecallEthanol{}) against
the control variate's \fsGateCVRecallEthanol, while skipping only \fsGateEnergySkipEthanol{} of
evaluations against \fsGateCVSkipEthanol. Elsewhere that free gate is the informative failure: cheap
but coarse, skipping \fsGateEnergySkipThreeBPA{} on 3BPA and collapsing to
\fsGateEnergyRecallAzobenzene{} \fsGateEnergyRecallAzobenzeneCI{} recall on azobenzene, so it cannot
be trusted as a safety screen --- energy disagreement is a genuinely different signal from force
disagreement, not a cheap proxy for it. Head subsampling with the exact mean is statistically almost
indistinguishable from the Haar sketch at equal budget, as their nearly equal estimator variances
predict; what separates the control variate from both is exploiting the concentration in the
head-space spectrum rather than probing it obliviously. All three spend identical lanes and all are
net speedups at $B{=}16$, so that margin is decision quality, not cost.


\section{Conclusion and limitations}
\label{sec:limitations}
Exact multi-head force uncertainty cannot be replaced by a handful of randomized head-space probes at
...
Whether it saves \emph{time} is separate, and the honest answer is: only in batch. Once batched
reverse mode is available, extra cotangent lanes are nearly free at $B{=}1$, the total speedup falls
to $\fsBestTotalKThreeBOne\times$ and the gate becomes a net slowdown
($\fsBestGateSpeedupBOne\times$); single-structure MD is precisely where this does not pay for
itself. The batched advantage is bounded above too ($\fsBatchedWinBOne\times$ at $B{=}1$,
$\fsBatchedWinBSixteen\times$ at $B{=}16$, slower than serial by $B{=}\fsBatchedLosesBatch$), a
crossover measured on one 8\,GB card without a claimed mechanism. We also preserve a \emph{proxy},
not a demonstrated outcome: we run no active-learning loop and measure no reference-force error, so
we claim no gain in model quality or DFT-query efficiency. Further: \texttt{torch.func} is
unavailable here; committee disagreement is only a proxy for error; calibration is marginal and
under exchangeability, not out of distribution; $M{=}8$ bounds the maximum saving; and rMD17 spans
only 9--24 atoms, so molecule size is a weak axis. The method therefore suits batched
active-learning sweeps, not single-structure MD steps --- and that boundary, with the fact that
orthogonalizing the probes costs nothing and buys more than any other estimator choice, is what we
think is worth taking away.
...
% ICBINB-BIO does not ask for the NeurIPS checklist; omitted from this build.
