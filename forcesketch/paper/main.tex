...
\newcommand{\fsCodeUrl}{https://anonymous.4open.science/r/PLACEHOLDER}
...
result is robust; the speed result is not.
\end{abstract}
...
when an expensive reference calculation is required. Committee disagreement remains the most widely
used estimator~\citep{behler2015,podryabinkin2017,carrete2023}, and multi-head
committees~\citep{beck2025mhc} --- shallow ensembles more broadly~\citep{kellner2024} --- make it
dramatically cheaper by attaching $M$ readout heads to one shared message-passing trunk, so all $M$
energies come from a single forward pass.
...
disguises the cause. Appendix~\ref{app:tensorexpr} diagnoses it and Appendix~\ref{app:compile} shows
