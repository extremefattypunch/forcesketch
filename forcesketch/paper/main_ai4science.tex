...
% AI for Science workshop (NeurIPS 2026) build. The call for papers asks to
% "Change the template footnote to 'Submitted to ... the AI for Science workshop
% (NeurIPS 2026).'" In this style file that footnote is gated on \if@workshop,
% which is set by the `dblblindworkshop' option (neurips_2026.sty:90). That
% option deliberately does NOT touch \if@anonymous -- only `sglblindworkshop'
% adds \@anonymousfalse -- so the anonymous author block is retained. The
% workshop does not state a blinding policy; anonymising a non-blind venue costs
% nothing, whereas de-anonymising a blind one is grounds for desk rejection.
...
result is robust; the speed result is not. Read as a question about verification, the split is
the informative part: a noisy surrogate verifier cannot stand in for an exact one when the task is
to rank, but it can be calibrated to decide, at a controlled error rate, which cases need the exact
verifier at all.
\end{abstract}
...
\paragraph{Verification under an imperfect simulator.}
A learned potential is a surrogate for a quantum-chemistry reference it can only approximate, so
every trajectory it produces raises the question of when its output can be trusted in place of that
reference. Committee disagreement is the verifier that answers it, and force disagreement is the
part of that verifier molecular dynamics actually consumes --- which makes the cost of the verifier,
not the cost of the model, the binding constraint on how often we can afford to check. The question
this paper asks is the sharper one of whether a cheaper, noisier verifier can substitute for the
exact one, and our answer separates the two ways it might: it cannot reproduce the exact verifier's
ranking, but it can be calibrated to decide which structures need the exact verifier at all.
...
% The AI for Science call for papers states the NeurIPS checklist is not
% required for this workshop, so it is omitted from this build.
